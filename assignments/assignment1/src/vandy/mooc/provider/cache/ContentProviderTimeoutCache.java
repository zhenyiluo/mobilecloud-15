 ...
 */
public class ContentProviderTimeoutCache implements
		TimeoutCache<String, List<AcronymExpansion>> {
	/**
	 ...
	 * schedule Cache cleanup at regular intervals to remove expired Acronym
	 * Expansions.
	 */
	...
	public List<AcronymExpansion> get(final String acronym) {
		...
		final String SELECTION_ACRONYM = AcronymEntry.COLUMN_ACRONYM + " = ?";
		...
		try (Cursor cursor = mContext.getContentResolver().query(
				AcronymEntry.CONTENT_URI, null, SELECTION_ACRONYM,
				selectionArgs, null)) {
			...
			// from first row and add it to the List.
				...
				// TODO -- repl ace "0" with the expiration time of
				// given acronym that's obtained from the cursor.
				Long expirationTime = cursor.getLong(cursor
						.getColumnIndex(AcronymEntry.COLUMN_EXPIRATION_TIME));
				...
				if (System.nanoTime() > expirationTime) {
					...
					List<AcronymExpansion> longForms = new ArrayList<AcronymExpansion>();
	...
	private AcronymExpansion getAcronymExpansion(Cursor cursor) {
		...
		String longForm = cursor.getString(cursor
				.getColumnIndex(AcronymEntry.COLUMN_LONG_FORM));
		// TODO -- replace "0" with the "frequency" value of the acronym
		// obtained from the cursor.
		int frequency = cursor.getInt(cursor
				.getColumnIndex(AcronymEntry.COLUMN_FREQUENCY));
		// TODO -- replace "0" with the "since" value of the acronym
		// obtained from the cursor.
		int since = cursor.getInt(cursor
				.getColumnIndex(AcronymEntry.COLUMN_SINCE));
	...
	/**
	 * Put the @a longForms into the Database at the designated @a acronym with
	 * a certain timeout after which the cached data will expire.
	 * 
	 * @param acronym
	 * @param longForms
	 * @param timeout
	 ...
	 */
	private int putValues(String acronym, List<AcronymExpansion> longForms,
			long timeout) {
		...
		Long expirationTime = System.nanoTime() + timeout;
		...
		ContentValues[] cvArray = new ContentValues[longForms.size()];
		...
		for (int i = 0; i < longForms.size(); i++) {
			...
			AcronymExpansion ae = longForms.get(i);
			cv.put(AcronymEntry.COLUMN_ACRONYM, acronym);
			cv.put(AcronymEntry.COLUMN_EXPIRATION_TIME, expirationTime);
			cv.put(AcronymEntry.COLUMN_FREQUENCY, ae.getFreq());
			cv.put(AcronymEntry.COLUMN_LONG_FORM, ae.getLf());
			cv.put(AcronymEntry.COLUMN_SINCE, ae.getSince());
			cvArray[i] = cv;
		}
		...
		return mContext.getContentResolver().bulkInsert(
				AcronymEntry.CONTENT_URI, cvArray);
	}
	...
	public int size() {
		...
		// of Acronym table.
		try (Cursor cursor = mContext.getContentResolver().query(
				AcronymEntry.CONTENT_URI, null, null, null, null)) {
	...
	public void removeExpiredAcronyms() {
		...
		// expired.
		final String SELECTION_EXPIRATION = AcronymEntry.COLUMN_EXPIRATION_TIME
				+ " <= ?";

		String[] selectionArgs = { String.valueOf(System.nanoTime()) };
	...
	private boolean isAlarmActive(Context context) {
		return DeleteCacheReceiver.makeCheckAlarmPendingIntent(context) != null;
	}
}
